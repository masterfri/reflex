...
Expression.prototype = 
{
	...
	getString: function()
	{
		var str = '';
		this.expect("'");
		while (! this.isTerminator() && "'" != this.sym) {
			if ('\\' == this.sym) {
				this.getChar();
			}
			str += this.sym;
	...
	{	
		...
		if (! this.isTerminator() && this.isAlpha(this.sym)) {
			tok += this.sym;
			this.getChar();
			while (! this.isTerminator() && (this.isAlNum(this.sym) || '.' == this.sym)) {
				tok += this.sym;
				if ('.' == this) {
					this.getChar();
					if (this.isTerminator() || ! this.isAlpha(this.sym)) {
						throw 'alpha character expected';
					}
				} else {
					this.getChar();
				}
...
Expression.libs = {
	...
	'array': {
		...
		'array_push': function (a, x) { a.push(x); return a; },
		'array_pop': function (a) { return a.pop(); },
		'array_remove': function (a, x) { var i = a.indexOf(x); if (i != -1) a.splice(i, 1); return a; },
		'array_set': function (a, i, x) { a[i] = x; return a; },
		'array_get': function (a, i) { return a[i]; },
		'array_contains': function (a, x) { return a.indexOf(x) != -1; },
		'array_join': function (a, s) { return a.join(s); }
	}
};
